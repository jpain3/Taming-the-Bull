...
PirateBeerGame_Opt = function(AI_Entity = FALSE, AI_Entity_Index, AI_parameter_df = FALSE, AI_Order = FALSE, 
                          t, Orders, 
  ...
  if (unname(AI_parameter_df[1] != FALSE)) {
  ...
  reward = sum(-25*Amplification^2 + 1)
  
  
  fnt_output = list(Order_flows, Shipping_flows, OH_Inventory, Backorder,
                    L, L_hat, Production_Request, Amplification, reward)
